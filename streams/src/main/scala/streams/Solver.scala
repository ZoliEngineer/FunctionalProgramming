...
trait Solver extends GameDef {
  ...
  def newNeighborsOnly(neighbors: Stream[(Block, List[Move])],
          ...
          neighbors.filterNot(neighborWithHistory => explored.contains(neighborWithHistory._1))
   ...
   */
  lazy val pathsToGoal: Stream[(Block, List[Move])] = pathsFromStart.filter(step => done(step._1))
